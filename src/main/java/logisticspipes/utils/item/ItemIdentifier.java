...
public final class ItemIdentifier implements Comparable<ItemIdentifier> {
  ...
  private static class ItemKey {
    ...
    public final DataComponentMap dataComponents;

    public ItemKey(Item item, DataComponentMap dataComponents) {
      this.item = item;
      this.itemDamage = dataComponents.getOrDefault(DataComponents.DAMAGE, 0);
      this.dataComponents = dataComponents;
    }

    @Override
    public boolean equals(Object that) {
      if (!(that instanceof ItemKey i)) {
        return false;
      }
      return item == i.item && itemDamage == i.itemDamage && dataComponents.equals(i.dataComponents);
    }

    @Override
    public int hashCode() {
      return item.hashCode() ^ itemDamage ^ dataComponents.hashCode();
    }
  }
  ...
  private static class MapDamagedIdentifierHolder implements IDamagedIdentifierHolder {

    private final ConcurrentHashMap<Integer, ItemIdentifier> holder;

    public MapDamagedIdentifierHolder() {
  ...
  private static class ArrayDamagedIdentifierHolder implements IDamagedIdentifierHolder {

    private AtomicReferenceArray<ItemIdentifier> holder;

    public ArrayDamagedIdentifierHolder(int damage) {
  ...
  public final Item item;
  public final int itemDamage;
  public final DataComponents tag;
  ...
  private ItemIdentifier(Item item, int itemDamage, FinalNBTTagCompound tag, int uniqueID) {
    ...
    this.modName = BuiltInRegistries.ITEM.getKey(item).getNamespace();
  }

  private static ItemIdentifier getOrCreateSimple(Item item, @Nullable ItemIdentifier proposal) {
    if (proposal != null) {
  ...
  private static ItemIdentifier getOrCreateDamage(Item item, int damage, ItemIdentifier proposal) {
    ...
    if (damages == null) {
      if (item.getMaxDamage() < 32767) {
        damages = new ArrayDamagedIdentifierHolder(damage);
      } else {
        damages = new MapDamagedIdentifierHolder();
      }
  ...
  private static ItemIdentifier getOrCreateTag(Item item, int damage, DataComponentMap tag) {
    ItemKey k = new ItemKey(item, tag);
    ...
    ItemKey realKey = new ItemKey(item, finaltag);
  ...
  public static ItemIdentifier get(Item item, int itemUndamagableDamage, DataComponentMap tag) {
    return get(item, itemUndamagableDamage, tag, null);
  }

  private static ItemIdentifier get(Item item, int itemUndamagableDamage, DataComponentMap tag, ItemIdentifier proposal) {
    ...
    if (tag == null && itemUndamagableDamage == 0) {
      ...
      //tag
      return ItemIdentifier.getOrCreateTag(item, itemUndamagableDamage, tag);
    }
  }
  ...
  public static ItemIdentifier get( ItemStack itemStack) {
    ...
    ItemIdentifier ident = ItemIdentifier.get(itemStack.getItem(), itemStack.getItemDamage(),
        itemStack.getComponents(), proposal);
...
/* Instance Methods *//*
  ...
  public String getDebugName() {
    return item.getDescriptionId() + "(ID: " + Item.getId(item) + ", Damage: " + itemDamage + ")";
  }
  ...
  public ItemStack unsafeMakeNormalStack(int stackSize) {
    ItemStack stack = new ItemStack(item, stackSize, tag);
    stack.set(tag);
    return stack;
  }
...
/*private static Map<Integer, Object> getArrayAsMap(int[] array) {
  ...
  public int compareTo(ItemIdentifier o) {
    int c = Item.getId(item) - Item.getId(o.item);
    if (c != 0) {
      return c;
    }
